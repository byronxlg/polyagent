...
def test_get_principals(db_session: Session, override_session_local):
    """Test getting all principals."""
    service = PrincipalService()
    principals = service.get_principals()

    # Should have at least the system principal
    assert len(principals) >= 1
    assert all(isinstance(p, Principal) for p in principals)


def test_get_principals_filter_by_type(db_session: Session, override_session_local):
